...
class DFS {
    ...
    stackRemove() {
        // get last element
        let last = Array.from(this.stack).pop();
        this.stack.delete(last);
    }

    get currentStackVertex() {
        return Array.from(this.stack).pop();
    }

    search() {
        let paths = [];
        // get length of graph
        let graphLength = this.nodesCount ;
        let neighbors = [];
        let currentVertex = this.rawNodes[0][0];
        this.stackAdd(this.rawNodes[0][0]);
        ...
        for(let i = 0 ; i < graphLength; i++) {
            ...
            this.stackRemove();
            // add current vertex and neighbors to the stack
            for(let j = 0 ; j < neighbors.length; j++) {
                this.stackAdd(neighbors[j][1]);
            }
            // get current(last) vertex from stack
            currentVertex = this.currentStackVertex;
        }
        return paths;
    }
...
let dfs = new DFS(nodes);
let paths = dfs.search();
